...
# def rank_jobs(resume_text, top_n=5, skill_boost_factor=2):
#     """
#     Ranks jobs using a hybrid approach:
#     1. Base score from semantic (cosine) similarity.
#     2. Boosted score for each matching skill.
#     """
#     if jobs_df is None or job_embeddings is None or not isinstance(resume_text, str) or not resume_text:
#         print("Error: Required data or resume text is missing for ranking.")
#         return []

#     # Get skills from the user's resume
#     user_skills = extract_skills_from_text(resume_text)

#     # Calculate the base semantic scores
...
#         boosted_score = cosine_scores[i].item() * 100 + (len(matching_skills) * skill_boost_factor)
...
#     # Sort the results by the new boosted score
...
#             # Debugging print statement for extracted text
...
#         print("Please run 'python setup_embeddings.py' first to prepare data.")
#         print("*********************************************************")
#     else:
#         app.run(debug=True)
# File: app.py
